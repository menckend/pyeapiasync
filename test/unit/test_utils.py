...
class TestUtils(unittest.TestCase):
    ...
    def test_load_module(self, mock_import_module):
        ...
        result = pyeapiasync.utils.load_module('test')
    ...
    def test_load_module_raises_import_error(self, mock_import_module):
        ...
        with self.assertRaises(ImportError):
            pyeapiasync.utils.load_module('test')

    def test_make_iterable_from_string(self):
        result = pyeapiasync.utils.make_iterable('test')
        self.assertIsInstance(result, Iterable)
        self.assertEqual(len(result), 1)

    def test_make_iterable_from_unicode(self):
        result = pyeapiasync.utils.make_iterable(u'test')
        self.assertIsInstance(result, Iterable)
        self.assertEqual(len(result), 1)
    ...
    def test_make_iterable_raises_type_error(self):
        with self.assertRaises(TypeError):
            pyeapiasync.utils.make_iterable(object())

    def test_import_module(self):
        result = pyeapiasync.utils.import_module('pyeapi.api.vlans')
        self.assertIsNotNone(result)

    def test_import_module_raises_import_error(self):
        with self.assertRaises(ImportError):
            pyeapiasync.utils.import_module('fake.module.test')

    def test_expand_singles(self):
        vlans = '1,2,3'
        result = pyeapiasync.utils.expand_range(vlans)
        result = ','.join(result)
    ...
    def test_collapse_singles(self):
        vlans = '1,3,5,7'
        result = pyeapiasync.utils.collapse_range(vlans)
    ...
    def test_debug(self, mock_logger):
        pyeapiasync.utils.islocalconnection = Mock(return_value=True)
        pyeapiasync.utils.debug('test')
